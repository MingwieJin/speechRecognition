...
class IatRecorder {
  ...
  connectWebsocket () {
    ...
    // var url = 'ws://36.7.109.47:19070/ws/aiability/v1/ist'
...
var keyWrod = ['蓝色', '定制效果', '穿什么', '就选他', '点上吧', '车都备好了', '约上吧', '两个人的位置'];
...
class IatTaste {
  ...
  setResult (data) {
    ...
    rtasrResult.forEach(i => {
      ...
      } else if (currentText.length !== 0 && str !== '') {
  ...
  getResponse (str) {
    console.log(123123, str, keyWrod[0])
    // var keyWrod = ['蓝色', '定制效果', '穿什么', '就选他', '点上吧', '车都备好了', '约上吧', '两个人的位置'];

    if (str.indexOf(keyWrod[0]) !== -1) {
      keyWrod.splice(0, 1)
      let frame = document.getElementById('dialog-iframe');
      frame.contentWindow.postMessage({ data: 'start' }, '*')
      top.postMessage({ data: 'start' }, '*');
    }
    // if (str.indexOf('智慧小东') !== -1 ) {
    //   // console.log(11112344444, $("#dialog-iframe"), $("#dialog-iframe").contents(), $("#dialog-iframe").contents().find("#my-audio-ctrl-btn"))
    //   let frame = document.getElementById('dialog-iframe');
    //   frame.contentWindow.postMessage ({ data: 'start'}, '*')
    // }
  }
